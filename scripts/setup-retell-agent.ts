// One-time setup: creates the Retell LLM + Agent via the API rather than
// clicking through the dashboard, so the configuration is reproducible and
// reviewable. Run via `npm run setup:retell`. Safe to re-run — it always
// creates a new LLM+agent version rather than mutating in place, since
// Retell's update-in-place API would need the ids from a prior run anyway.
...
Important:
- Never give medical or clinical advice. If asked about symptoms, treatment specifics, or medication, say the dentist will go over that in person at the visit.
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            treatment: {
              type: "string",
              description: "The treatment they need.",
              enum: [
                "Teeth Cleaning",
                "Fillings",
                "Crowns & Bridges",
      ...
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            treatmentType: { type: "string", description: "The treatment being booked for, if known." },
          },
        },
      },
